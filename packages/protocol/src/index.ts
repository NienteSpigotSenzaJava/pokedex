...
export const WorkspaceSchema = z.object({
  alias: z.string().min(1).regex(/^[a-z0-9][a-z0-9_-]{0,63}$/i),
  root: z.string().min(1),
...
export const stableCapabilities: Capability[] = [
  { name: "native_threads", level: "stable", reason: "uses codex app-server thread APIs", source: "app_server" },
  { name: "resume", level: "stable", reason: "thread/resume appends turns to stored local codex threads", source: "app_server" },
  { name: "fork", level: "stable", reason: "thread/fork copies stored local thread history", source: "app_server" },
  { name: "goals", level: "stable", reason: "thread/goal methods are exposed by app-server", source: "app_server" },
  { name: "review", level: "stable", reason: "review/start runs codex reviewer on a native thread", source: "app_server" },
  { name: "diff", level: "adapter", reason: "git diff is read locally from allowed workspaces", source: "cli" }
];
