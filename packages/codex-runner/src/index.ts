...
import { assertSandboxAllowed, findWorkspace, redactSecrets, resolveWorkspaceRoot } from "@pokedex/security";
...
export class CodexAppServerClient {
  ...
  async setupCheck(config: AgentConfig): Promise<ToolResult> {
    ...
    return ToolResultSchema.parse({
      ...
      summary: doctor.exitCode === 0 ? "codex setup looks usable." : "codex doctor reported setup issues.",
      data: {
  ...
  async listThreads(config: AgentConfig, input: unknown): Promise<ToolResult> {
    ...
    if (args.workspaceAlias) params.cwd = resolveWorkspaceRoot(findWorkspace(config, args.workspaceAlias));
  ...
  async startThread(config: AgentConfig, input: unknown, onProgress?: (progress: RunnerProgress) => void): Promise<RunnerResult> {
    ...
    try {
      const started = await this.request(config, "thread/start", stripUndefined({ cwd, name: task.name, settings }));
  ...
  async startTurn(config: AgentConfig, input: unknown, onProgress?: (progress: RunnerProgress) => void): Promise<RunnerResult> {
    ...
    try {
      const result = await this.request(config, "turn/start", {
        ...
        input: [...task.skills.map(({ name, path }) => ({ type: "skill", name, path })), { type: "text", text: task.prompt }],
  ...
  async resumeThread(config: AgentConfig, input: unknown, onProgress?: (progress: RunnerProgress) => void): Promise<RunnerResult> {
    const task = TurnStartSchema.parse(input);
  ...
  async review(config: AgentConfig, input: unknown, onProgress?: (progress: RunnerProgress) => void): Promise<RunnerResult> {
    ...
    const threadId =
      ...
      (
        await this.startThread(
          config,
          stripUndefined({ workspaceAlias: task.workspaceAlias, prompt: task.prompt, model: task.model, profile: task.profile, reasoningEffort: task.reasoningEffort, verbosity: task.verbosity, sandbox: task.sandbox, approvalPolicy: task.approvalPolicy, webSearch: task.webSearch }),
          onProgress
        )
      ).threadId;
    ...
    try {
      ...
      return { threadId, finalMessage: extractFinalMessage(result) || "review started.", usage: extractUsage(result) ?? UsageSchema.parse({}), events };
    } finally {
      off();
    }
  }
  ...
  async request(config: AgentConfig, method: string, params: unknown, timeoutMs = 120_000): Promise<unknown> {
  ...
  private collect(events: unknown[], onProgress?: (progress: RunnerProgress) => void, threadId?: string): () => void {
  ...
  private ensureStarted(config: AgentConfig): void {
    ...
    this.child.on("close", (code) => {
      this.failAll(new Error(`codex app-server exited with ${code}: ${this.stderr}`));
      this.child = null;
      this.initialized = false;
      this.initializing = null;
    });
...
export function buildSettings(config: AgentConfig, workspace: Workspace, task: RuntimeSettings): JsonRecord {
...
export async function diffResult(config: AgentConfig, input: unknown): Promise<ToolResult> {
  ...
  const [stat, names] = await Promise.all([runPlainCommand({ command: "git", args: ["diff", "--stat"], cwd: root, env: process.env }), runPlainCommand({ command: "git", args: ["diff", "--name-only"], cwd: root, env: process.env })]);
  return { ok: stat.exitCode === 0 && names.exitCode === 0, summary: stat.stdout.trim() || "no diff.", data: { stat: stat.stdout, files: names.stdout.split(/\r?\n/).filter(Boolean) } };
}

export async function runPlainCommand(command: { command: string; args: string[]; cwd: string; env: NodeJS.ProcessEnv }): Promise<{ exitCode: number; stdout: string; stderr: string }> {
  let stdout = "";
  let stderr = "";

  const exitCode = await new Promise<number>((resolve, reject) => {
    const child = spawn(command.command, command.args, { cwd: command.cwd, env: command.env, stdio: ["ignore", "pipe", "pipe"] });
...
function extractThreadId(value: unknown): string | undefined {
  ...
  return stringFrom(raw.threadId) ?? stringFrom(raw.id) ?? stringFrom(thread.id) ?? stringFrom(thread.threadId);
}
...
function extractFinalMessage(value: unknown): string {
  ...
  return stringFrom(raw.finalMessage) ?? stringFrom(raw.output_text) ?? stringFrom(output.text) ?? stringFrom(item.text) ?? stringFrom(params.text) ?? "";
}
