...
const command = ["help", "--help", "-h"].includes(args[0] ?? "") ? "help" : args[0]?.startsWith("-") ? "local" : (args[0] ?? "local");
...
async function local() {
  ...
  config = createConfig(loadPokedexConfig());
  saveConfig();
...
function createConfig(old) {
  const first = firstWorkspace(old);
  const readOnly = has("--read-only");
  const writeEnabled = readOnly ? false : has("--write") || has("--full-access") ? true : (old.writeTasksEnabled ?? first.allowWrite ?? false);
  const fullAccess = readOnly ? false : has("--full-access") ? true : has("--write") ? false : (old.fullAccessEnabled ?? first.allowFullAccess ?? false);
  const alias = value("--alias") ?? old.alias ?? first.alias ?? "main";
  const root = value("--workspace") ? resolveUserPath(value("--workspace")) : (first.root ?? resolveUserPath("."));
  const workspaces = normalizeWorkspaces(old.workspaces, { alias, root, writeEnabled, fullAccess });
  ...
  return {
    port: String(value("--port") ?? old.port ?? "3000"),
    userId: value("--user-id") ?? old.userId ?? "local",
    relayUrl: `ws://127.0.0.1:${value("--port") ?? old.port ?? "3000"}/agent`,
    relayToken: value("--token") ?? old.relayToken ?? old.token ?? randomHex(),
    appServerCommand: value("--codex") ?? old.appServerCommand ?? "codex",
    appServerArgs: Array.isArray(old.appServerArgs) ? old.appServerArgs : ["app-server", "--listen", "stdio://"],
    defaultModel: value("--model") ?? old.defaultModel ?? "gpt-5.5",
    defaultReasoning: value("--reasoning") ?? old.defaultReasoning ?? "medium",
    defaultVerbosity: old.defaultVerbosity ?? "medium",
    defaultApprovalPolicy: old.defaultApprovalPolicy ?? "on-request",
...
function normalizeWorkspaces(workspaces, defaults) {
  ...
  const normalized = items
    .filter((workspace) => workspace && typeof workspace === "object" && workspace.alias && workspace.root)
    .map((workspace) => ({
      alias: String(workspace.alias),
      root: String(workspace.root),
      description: workspace.description ? String(workspace.description) : `${workspace.alias} workspace`,
      allowWrite: Boolean(workspace.allowWrite),
      allowFullAccess: Boolean(workspace.allowFullAccess),
      defaultSandbox: workspace.defaultSandbox ?? sandboxFor(workspace.allowWrite, workspace.allowFullAccess)
    }));
...
async function waitForRelay() {
  ...
  }, 10_000, "relay did not become healthy", "relay");
}
...
async function waitForAgent() {
  ...
  }, 15_000, "agent did not connect to relay", "agent");
}

async function waitForPoke() {
  await sleep(1400);
  const child = managedChildren.get("poke")?.child;
  if (!child || child.exitCode !== null || child.killed) throw serviceFailure("poke", "poke tunnel exited during startup");
}
...
async function handleCommand(parts) {
  ...
  if (name === "logs") return printLogs(subcommand);
  ...
  if (name === "reasoning") return await setEnum("defaultReasoning", subcommand, ["minimal", "low", "medium", "high", "xhigh"]);
  if (name === "verbosity") return await setEnum("defaultVerbosity", subcommand, ["low", "medium", "high"]);
  if (name === "approval") return await setEnum("defaultApprovalPolicy", subcommand, ["untrusted", "on-request", "never"]);
...
async function handleWorkspaceCommand(subcommand, rest) {
  ...
  if (subcommand === "full-access") return await setWorkspaceAccess(rest[0], "allowFullAccess", rest[1]);
...
async function setCodex(parts) {
  ...
  config.appServerArgs = parts.slice(1).length ? parts.slice(1) : ["app-server", "--listen", "stdio://"];
...
function printStatus() {
  console.log(`relay ${statuses.relay}`);
  console.log(`agent ${statuses.agent}`);
  console.log(`poke  ${statuses.poke}`);
  console.log(`mcp   ${mcpHttpUrl()}`);
  console.log(`mode  ${modeLabel(activeWorkspace())}`);
  console.log(`ws    ${activeWorkspace().alias} -> ${activeWorkspace().root}`);
}
...
function failIfServiceExited(name, fallback) {
  const entry = managedChildren.get(name);
  if (!entry) throw serviceFailure(name, fallback);
  if (entry.child.exitCode !== null || entry.child.killed || statuses[name] === "error") {
    throw serviceFailure(name, fallback);
  }
}

function serviceFailure(name, fallback) {
  if (!name) return new Error(fallback);
  const entry = managedChildren.get(name);
  const logs = formatLogs(name);
  if (!entry || (entry.child.exitCode === null && !entry.child.killed && entry.exitCode === null && !entry.signal && statuses[name] !== "error")) {
    return new Error(`${fallback}${logs ? `\n${logs}` : ""}`);
  }
  const status = entry?.signal ? `signal ${entry.signal}` : `code ${entry?.exitCode ?? entry?.child.exitCode ?? "unknown"}`;
  return new Error(`${name} exited with ${status}${logs ? `\n${logs}` : `\n${fallback}`}`);
}

function formatLogs(name) {
  const lines = serviceLogs.get(name) ?? [];
  return lines.length ? `${name} logs:\n${lines.slice(-20).join("\n")}` : "";
}
...
function loadPokedexConfig() {
  if (existsSync(configPath)) return readJson(configPath);
  // migrate old cwd-local config once, then save future state in ~/.pokedex/config.json.
  const oldConfig = resolveUserPath(".pokedex.json");
  const oldLocalState = resolveUserPath(".pokedex.local.json");
  const oldAgentConfig = resolveUserPath(".pokedex.agent.config.json");
  if (existsSync(oldConfig) || existsSync(oldLocalState) || existsSync(oldAgentConfig)) {
    return { ...readJson(oldAgentConfig), ...readJson(oldLocalState), ...readJson(oldConfig) };
  }
  return {};
}
...
function parseOnOff(raw, fallback) {
...
function assertAlias(alias) {
  if (!/^[a-z0-9][a-z0-9_-]{0,63}$/i.test(alias ?? "")) throw new Error("alias must match /^[a-z0-9][a-z0-9_-]{0,63}$/i");
}
...
function commandFor(bin) {
  return localFallback(bin) ?? { command: bin, args: [] };
}

function localFallback(bin) {
  ...
  if (packagedPaths[bin] && existsSync(packagedPaths[bin])) return { command: process.execPath, args: [packagedPaths[bin]] };
  if (paths[bin] && existsSync(paths[bin])) return { command: process.execPath, args: [paths[bin]] };
  return null;
}
...
function ensureLocalBuild(root, packagedPaths) {
  ...
  if (result.status !== 0) throw new Error(`local build failed. run npm run build and retry.${formatBuildOutput(result)}`);
}
