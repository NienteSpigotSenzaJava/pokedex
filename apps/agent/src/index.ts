...
import { AgentConfigSchema, AgentRequestSchema, type AgentConfig, type ToolResult } from "@pokedex/protocol";
...
function connect(): void {
  ...
  socket.on("message", async (raw) => {
    ...
    const result = await dispatch(request.toolName, request.arguments).catch((error: unknown): ToolResult => ({
      ok: false,
...
async function dispatch(toolName: string, args: Record<string, unknown>): Promise<ToolResult> {
  ...
  if (toolName === "pokedex_start_task") return runnerResultToTool("codex task started.", await codex.startThread(config, args));
  if (toolName === "pokedex_start_thread") return runnerResultToTool("codex thread started.", await codex.startThread(config, args));
  if (toolName === "pokedex_continue_task") return runnerResultToTool("codex task continued.", await codex.startTurn(config, args));
  if (toolName === "pokedex_send_turn") return runnerResultToTool("codex turn sent.", await codex.startTurn(config, args));
  if (toolName === "pokedex_resume_task") return runnerResultToTool("codex task resumed.", await codex.resumeThread(config, args));
  if (toolName === "pokedex_resume_thread") return runnerResultToTool("codex thread resumed.", await codex.resumeThread(config, args));
  ...
  if (toolName === "pokedex_review") return runnerResultToTool("codex review started.", await codex.review(config, args));
  ...
  if (toolName === "pokedex_get_usage") return runtime.lastUsage ?? { ok: false, summary: "no usage seen yet.", data: {} };

  return { ok: false, summary: `unknown tool: ${toolName}`, data: { capabilities: capabilitiesResult().data } };
}
...
function listWorkspaces(): ToolResult {
  return {
    ...
    data: {
      workspaces: config.workspaces.map(({ alias, description, allowWrite, allowFullAccess, defaultSandbox }) => ({
        alias,
        description,
        allowWrite,
        allowFullAccess,
...
function runnerResultToTool(summary: string, result: { threadId?: string; finalMessage: string; usage: unknown; events: unknown[] }): ToolResult {
  const toolResult = {
    ok: true,
    summary,
    data: {
  ...
  runtime.lastUsage = { ok: true, summary: "last usage loaded.", data: { threadId: result.threadId, usage: result.usage } };
  return toolResult;
}
